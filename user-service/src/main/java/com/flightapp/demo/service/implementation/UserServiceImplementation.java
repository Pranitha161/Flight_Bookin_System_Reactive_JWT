...
public class UserServiceImplementation implements UserService {
	...
	public Mono<ResponseEntity<String>> addUser(User user) {
		return userRepo.findByUsername(user.getUsername()).flatMap(
				existing -> Mono.just(ResponseEntity.status(HttpStatus.CONFLICT).body("Username already exists")))
				.switchIfEmpty(userRepo.findByEmail(user.getEmail()).flatMap(
						existing -> Mono.just(ResponseEntity.status(HttpStatus.CONFLICT).body("Email already exists")))
						.switchIfEmpty(Mono.defer(() -> {
							user.setPassword(passwordEncoder.encode(user.getPassword()));
							return userRepo.save(user).map(saved -> ResponseEntity.status(HttpStatus.CREATED)
	...
	public Mono<ResponseEntity<String>> login(LoginRequest loginRequest) {
		return userRepo.findByUsername(loginRequest.getUsername()).flatMap(existing -> {
			if (passwordEncoder.matches(loginRequest.getPassword(), existing.getPassword())) {
				...
				ResponseCookie cookie = ResponseCookie.from("jwt", token).httpOnly(true).secure(true).path("/")
						.maxAge(3600).build();
				return Mono.just(ResponseEntity.ok().header(HttpHeaders.SET_COOKIE, cookie.toString())
